...
__global__ void MatrixProblemK2(float *matriz, float *matrizResultado)
{
    // int tid = threadIdx.x;
    int tid = blockIdx.x * blockDim.x + threadIdx.x;

 	if(tid >= (TAM*TAM/2)-8 && tid < TAM*TAM/2){
      if(tid/TAM > 0 && tid/TAM < TAM-1 && tid%TAM != 0 && tid%TAM != TAM-1){
        matrizResultado[tid] = (0.25*(matriz[tid+1] + matriz[tid-1] + matriz[tid-TAM] + matriz[tid + TAM] - (4*matriz[tid]))) + matriz[tid];
    }
    else{
...
__global__ void MatrixProblem2(float *matriz, float *matrizResultado)
{
  	...
  	if(tid >= TAM*TAM/2 && tid < (TAM*TAM)+8){
      if(tid/TAM > 0 && tid/TAM < TAM-1 && tid%TAM != 0 && tid%TAM != TAM-1){
        matrizResultado[tid] = (0.25*(matriz[tid+1] + matriz[tid-1] + matriz[tid-TAM] + matriz[tid + TAM] - (4*matriz[tid]))) + matriz[tid];
    }
    else{
...
__global__ void MatrixProblem2K2(float *matriz, float *matrizResultado)
{
  	// int tid = threadIdx.x;
  	int tid = blockIdx.x * blockDim.x + threadIdx.x;

  	if(tid >= (TAM*TAM/2)+8 && tid <= TAM*TAM){
      if(tid/TAM > 0 && tid/TAM < TAM-1 && tid%TAM != 0 && tid%TAM != TAM-1){
        matrizResultado[tid] = (0.25*(matriz[tid+1] + matriz[tid-1] + matriz[tid-TAM] + matriz[tid + TAM] - (4*matriz[tid]))) + matriz[tid];
    }
    else{
...
int main(int argc, char **argv) {
    ...
    while(mdftime){
      if(id_task == MASTER){
        ...
        cudaMemcpyAsync(pMatriz, m0, sizeof(float) * size_t(TAM*TAM), cudaMemcpyHostToDevice, streams[2]); 
        MatrixProblemK2 <<<256, 1024,0, streams[2]>>>(pMatriz, pMatrizResultado);
        cudaDeviceSynchronize();
        cudaMemcpyAsync(m0, pMatrizResultado, sizeof(float) * size_t(TAM*TAM), cudaMemcpyDeviceToHost, streams[2]);
        ...
        cudaMemcpyAsync(pMatriz2, m0, sizeof(float) * size_t(TAM*TAM), cudaMemcpyHostToDevice, streams[3]); 
        MatrixProblem2 <<<256, 1024, 0, streams[3]>>>(pMatriz2, pMatrizResultado2);
        cudaDeviceSynchronize();
        cudaMemcpyAsync(m0, pMatrizResultado2, sizeof(float) * size_t(TAM*TAM), cudaMemcpyDeviceToHost, streams[3]);
